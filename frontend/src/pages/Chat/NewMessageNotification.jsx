...
import { Link } from "react-router-dom"; // Use react-router-dom's Link instead of @inertiajs/react Link
        ...
        <div className="toast toast-top toast-center min-w-[300px]">
            {toasts.map((toast) => (
                <Link
                    ...
                    to={
                        toast.group_id
                            ? `/chat/group/${toast.group_id}` // Using react-router-dom's `to` prop
                            : `/chat/user/${toast.user.id}`   // Using react-router-dom's `to` prop
                    }
