...
import { Link } from "react-router-dom"; // Use react-router-dom Link instead of @inertiajs/react Link
                        ...
                        <Popover.Panel className="absolute right-0 z-20 mt-3 w-[240px] px-4 sm:px-0">
                            <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black/5">
                                <div className="bg-gray-800 py-2">
                                    {users.map((user) => (
                                        <Link
                                            to={`/chat/user/${user.id}`} // Using react-router-dom's `to` for routing
                                            key={user.id}
