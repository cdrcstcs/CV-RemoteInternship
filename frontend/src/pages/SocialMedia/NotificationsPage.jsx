...
const NotificationsPage = () => {
  ...
  const renderNotificationIcon = (type) => {
    switch (type) {
      case "like":
        return <ThumbsUp className="text-emerald-400" />;
      case "comment":
        return <MessageSquare className="text-emerald-400" />;
      case "connectionAccepted":
        return <UserPlus className="text-emerald-400" />;
      default:
        return null;
    }
  };
    ...
    return (
      <>
        ...
        <div className="flex-1 overflow-hidden">
          <p className="text-sm text-emerald-400 truncate">{relatedPost.content}</p>
        </div>
          ...
          {isLoadingNotifications ? (
            ...
            <ul>
              {notifications.map((notification) => (
                <li
                  key={notification.id}
                  className={`bg-transparent border-2 border-white rounded-lg p-4 my-4 transition-all hover:shadow-md ${
                    !notification.read ? "border-emerald-400" : "border-gray-200"
                  }`}
                >
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm text-emerald-400">{renderNotificationContent(notification)}</p>
                        </div>
