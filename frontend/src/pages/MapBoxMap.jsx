...
const MapboxMap = ({ routeDetails }) => {
  ...
  useEffect(() => {
    if (routeDetails && routeDetails.length > 0) {
      const routePromises = routeDetails.flatMap((routeDetail) => {
        const { start_latitude, start_longitude, end_latitude, end_longitude, id } = routeDetail;

        // Only continue if we have coordinates for both start and end
          ...
          return fetch(routeRequestUrl)
            ...
            .then((data) => {
              if (data.routes && data.routes.length > 0) {
                // Assign a unique random color for each route
        ...
        return null; // Skip this route if coordinates are missing
      });

      Promise.all(routePromises).then((routeDetails) => {
        const validRouteDetails = routeDetails.filter((route) => route !== null);
        setRoutesData(validRouteDetails); // Only set valid routes
      });
  ...
  const getRouteColor = (routeId) => {
    return routeColors[routeId] || "#1e90ff"; // Default color if not set
  };
      ...
      <div style={{ height: "100%" }} className="rounded-lg">
        {routeDetails && routeDetails.length > 0 ? (
          <Map
            ...
            initialViewState={{
              longitude: routeDetails[0]?.start_longitude || -74.0060,
              latitude: routeDetails[0]?.start_latitude || 40.7128,
              zoom: 5,
            }}
            style={{ width: "100%", height: "100%", borderRadius: "10px" }} // Ensure the map takes full height of its parent container
            mapStyle="mapbox://styles/mapbox/streets-v9"
          >
            <Markers routeDetails={routeDetails} />
